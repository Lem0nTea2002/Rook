...
from rook_agent.utils.json_utils import dumps_json, loads_json_object
from rook_agent.providers.base import ChatProvider
from rook_agent.providers.errors import (
    ProviderError,
    ProviderErrorKind,
    classify_provider_error,
    classify_provider_exception,
)
from rook_agent.providers.tool_adapters import to_openai_tool
...
class OpenAICompatibleProvider(ChatProvider):
    ...
    def __init__(
        ...
        *,
        ...
        if transport_max_retries is not None and transport_max_retries < 0:
            raise ValueError("transport_max_retries 不能小于 0")
        self._name = name
        self._model = model
        ...
        if client is not None:
            ...
            if transport_max_retries is not None:
                kwargs["max_retries"] = transport_max_retries
            if base_url:
    ...
    def transport_max_retries(self) -> int | None:
        return self._transport_max_retries
    ...
    def _build_completion_params(self, request: ChatRequest) -> dict[str, Any]:
            ...
            "messages": [
                self._to_openai_message(
                    ...
                    include_reasoning_content=(
                        self._capabilities.requires_reasoning_content_for_tool_calls
                    ),
                )
        ...
        if request.tools:
            ...
            if self._capabilities.supports_tool_choice_parameter:
...
def _start_stream_worker(stream: Any) -> tuple[queue.Queue[Any], threading.Event]:
    ...
    threading.Thread(target=worker, name="rook-openai-stream", daemon=True).start()

...
def create_provider_from_config(
    ...
    *,
    ...
    if selected in {"openai-compatible", "custom"}:
        return _create_custom_openai_compatible(
            config,
            transport_max_retries=transport_max_retries,
        )
    ...
    if not model:
        raise ProviderConfigError(f"缺少模型配置：{preset.model_env} 或 config model")
    base_url = (
    ...
    trust_env = config.get_provider_bool(
        "trust_env",
        env="ROOK_PROVIDER_TRUST_ENV",
        default=True,
...
def _provider_api_key(config: AppConfig, fallback_env: str, *, provider_name: str) -> str | None:
    ...
    if configured_env and _ENVIRONMENT_VARIABLE_NAME.fullmatch(configured_env) is None:
        raise ProviderConfigError(
            "provider.api_key_env 必须是环境变量名称；"
            "请运行 rook config setup --force 重新配置。"
        )

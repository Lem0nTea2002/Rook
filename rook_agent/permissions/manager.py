...
class PermissionManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.session_grants = PermissionGrantStore()
        self.mode = mode
    ...
    def preflight(self, request: PermissionRequest) -> PermissionDecision:
        ...
        if self.mode == PermissionMode.FULL:
            return PermissionDecision(
                ...
                reason="full 模式跳过 Rook 权限策略，仅保留工具审计。",
            )
        session_decision = self.session_grants.matching_decision(request)
        if session_decision is not None:
            return replace(session_decision, persistence=PermissionPersistence.SESSION)
        ...
        return UserInputRequest(
            ...
            options=[
                ...
                *(
                    [
                        UserInputOption(
                            id=PermissionConfirmationChoice.ALLOW_SESSION_SAME_SCOPE.value,
    ...
    def resolve_confirmation(self, request: PermissionRequest, choice: str) -> PermissionDecision:
        ...
        if normalized == PermissionConfirmationChoice.ALLOW_SESSION_SAME_SCOPE:
            ...
            grant = PermissionGrant(
                id=f"session_grant_{request.id}",
                effect="allow",
                action=request.action,
                scope_type=scope.scope_type,
                scope_value=scope.scope_value,
                created_at=datetime.now(timezone.utc).isoformat(),
                reason="用户选择 allow for session。",
            )
            self.session_grants.add(grant)
            return PermissionDecision(
                kind=PermissionDecisionKind.ALLOW,
                persistence=PermissionPersistence.SESSION,
                reason="用户允许本会话同范围后续执行。",
                grant=grant,
            )
...
def _normalize_choice(choice: str) -> PermissionConfirmationChoice | None:
    ...
    numeric = {
        ...
        "3": PermissionConfirmationChoice.ALLOW_SESSION_SAME_SCOPE,
        "4": PermissionConfirmationChoice.ALLOW_ALWAYS_SAME_SCOPE,
    }

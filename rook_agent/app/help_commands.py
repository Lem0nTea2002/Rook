...
BUILTIN_COMMAND_SPECS = (
    ...
    CommandSpec(
        "/review",
        "通过 EvoAgent 提交只读工作区审阅",
        "Review",
        argument_hint="[workspace|range <from> <to>|commit <sha>]",
    ),
    CommandSpec(
        "/review-report",
        "查看审阅报告或显式选择 Finding 生成修复任务",
        "Review",
        argument_hint="<task-id> [--fix <n>]",
    ),
    CommandSpec(
        "/learn",
        "审阅失败恢复经验并选择记忆或 Forge 去向",
        "进化",
        argument_hint="[last|save|forge|dismiss|defect]",
    ),
    CommandSpec("/memory", "查看已确认的项目记忆", "进化"),
    CommandSpec("/remember", "显式保存一条项目规则", "进化", argument_hint="<rule>"),

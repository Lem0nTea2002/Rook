...
class RookRuntime:
    ...
    recovery_opportunities: RecoveryOpportunityStore
...
def create_rook_runtime(
    *,
    ...
    tool_schema_fingerprint = stable_json_hash(
        [asdict(tool.definition) for tool in resolved_tools],
        length=32,
    )
    project_memory = ProjectMemoryStore(
        project_path,
    ...
    recovery_opportunities = RecoveryOpportunityStore(resolved_data_root / "learning")
    ...
    current = CurrentSessionState(
        session,
        project_memory_loader=project_memory.render_context,
    )
    ...
    review_handler = ReviewCommandHandler(
        client=EvoAgentClient(EvoAgentConfig.from_app_config(resolved_app_config)),
        session=current,
    )
    ...
    if evolution_config.enabled:
        candidate_coordinator = LearningCoordinator(
            config=evolution_config,
            store=recovery_opportunities,
        )
    chat_runner = AgentChatRunner(
    ...
    learning_handler = LearningCommandHandler(
        current_session=current,
        opportunities=recovery_opportunities,
        memory=project_memory,
        provider_getter=lambda: chat_runner.provider,
        candidate_proposer=lambda trace: CandidateService(
            distiller=ExperienceDistiller(
                chat_runner.provider,
                max_skills=evolution_config.max_skills_per_task,
            ),
            store=forge_store,
            writer=current.session.writer,
            project_root=project_path,
            config=evolution_config,
        ).propose(trace),
    )
    ...
    registrations = (
        ...
        (permission_handler, command_specs("/mode", "/permissions")),
        (review_handler, command_specs("/review", "/review-report")),
        (learning_handler, command_specs("/learn", "/memory", "/remember")),

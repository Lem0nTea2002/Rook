...
from rook_agent.providers.types import ChatResponse, ChatStreamEvent
...
class CurrentSessionState:
    ...
    project_memory_loader: Callable[[], str] | None = None

    def __post_init__(self) -> None:
        self._apply_project_memory(self.session)

    def set_session(self, session: AgentSession) -> None:
        self._apply_project_memory(session)
        self.session = session

    def _apply_project_memory(self, session: AgentSession) -> None:
        if self.project_memory_loader is not None:
            session.project_memory_context = self.project_memory_loader()
    ...
    def permission_manager(self):
        return self.session.permission_manager
...
class AgentChatRunner:
    ...
    last_recovery_opportunities: tuple[RecoveryOpportunity, ...] = ()
    ...
    def _after_turn(self) -> None:
        if self.candidate_coordinator is None:
            self.last_recovery_opportunities = ()
            return
        try:
            result = self.candidate_coordinator.after_turn(self.current_session.session) or ()
            self.last_recovery_opportunities = tuple(
                item for item in result if isinstance(item, RecoveryOpportunity)
            )
        except Exception:
            self.last_recovery_opportunities = ()

    def _after_turn_if_complete(self) -> None:
        if self.last_pending_input is not None:
            self.last_recovery_opportunities = ()
            return
        self._after_turn()
    ...
    def cancel_current_turn(self) -> None:
        ...
        self.current_session.session.cancel_pending_permission_execution()
...
def _run_coroutine_in_thread(coro: Coroutine[Any, Any, Any]) -> Any:

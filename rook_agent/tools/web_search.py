...
from rook_agent.tools.types import Tool, ToolPermissionSpec, ToolResult, make_error_result, make_text_result
from rook_agent.utils.introspection import tool_from_function
from rook_agent.utils.json_utils import dumps_json, loads_json
...
def create_web_search_tool() -> Tool:
    ...
    def web_search(
        ...
        # 确定 provider 顺序
        provider = os.environ.get("ROOK_WEBSEARCH_PROVIDER") or FIRST_SEARCH_PROVIDER
    ...
    tool.definition.parameters["properties"]["livecrawl"]["enum"] = LIVECRAWL_VALUES
...
def _run_parallel_search(query: str, num_results: int, timeout: int) -> ToolResult | None:
    ...
    headers = {"Content-Type": "application/json", "User-Agent": "Rook/0.1"}
...
def _run_exa_search(
    ...
    req = request.Request(
        ...
        headers={"Accept": "application/json, text/event-stream", "Content-Type": "application/json", "User-Agent": "Rook/0.1"},
    )
    try:

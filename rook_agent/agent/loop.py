...
from rook_agent.agent.cancellation import AgentCancelledError, CancellationToken, cancellation_context
from rook_agent.agent.loop_limits import AgentLoopLimits, AgentLoopStopReason
from rook_agent.agent.session import AgentSession, PendingPermissionExecution
from rook_agent.agent.user_input import (
    AgentTurnResult,
    AgentTurnStatus,
    UserInputRequest,
...
from rook_agent.providers.types import ChatMessage, ChatRequest, ChatResponse, ChatStreamEvent, ToolCall
from rook_agent.skills.loader import SkillLoadError, SkillLoader
from rook_agent.skills.router import SkillRouter
from rook_agent.skills.session import append_skill_loaded, append_skill_required_file_loaded, append_skill_selected
from rook_agent.tools.permission_results import make_permission_denied_result
...
_WORKSPACE_MUTATION_TOOL_NAMES = frozenset({"write", "edit", "delete", "apply_patch"})
...
def _has_discarded_tool_calls(response: ChatResponse) -> bool:
    if response.finish_reason != "tool_calls" or response.tool_calls:
        return False
    return any("已丢弃整组不可执行调用" in warning for warning in response.diagnostics.warnings)


def _discarded_tool_call_retry_prompt(response: ChatResponse) -> str:
    if any("阶段预算拒绝未暴露工具" in warning for warning in response.diagnostics.warnings):
        return (
            "上一条响应请求了当前阶段未暴露工具，该调用没有执行。"
            "仅有这一次纠正机会；必须从本次暴露的工具中选择并立即执行。"
        )
    return (
        "上一条工具调用参数不是合法 JSON object，整组调用未执行。"
        "请仅重试一次，并严格按照工具 schema 传入 JSON object。"
    )


def _discarded_tool_call_terminal_message(response: ChatResponse) -> str:
    if any("阶段预算拒绝未暴露工具" in warning for warning in response.diagnostics.warnings):
        return "模型连续两次请求当前阶段未暴露工具，Rook 已安全结束本轮。"
    return "模型连续两次返回不可执行的工具参数，Rook 已安全结束本轮。"
...
class AgentLoop:
    ...
    def __init__(
        ...
        *,
        ...
        self._tool_failure_states: dict[str, _ToolFailureState] = {}
        self._budget_finalization_requested = False
    ...
    def _append_permission_resume_result(self, request_id: str, answer: str) -> AgentTurnResult | None:
        ...
        if decision.kind == PermissionDecisionKind.DENY:
            ...
            self._record_tool_result_for_repeat_guard(pending.tool_call, result)
            self._emit_tool_event(
                "finished",
                pending.tool_call,
                result=result,
    ...
    def _complete_once(self, *, tool_choice="auto") -> ChatResponse:
        ...
        tool_choice = self._reserve_final_provider_call(tool_choice)
    ...
    def _retry_discarded_tool_calls(
        self,
        response: ChatResponse,
        complete_once,
    ) -> ChatResponse:
        if not _has_discarded_tool_calls(response):
            return response
        self.session.append_assistant_response(response)
        self.session.append_runtime_control_message(
            _discarded_tool_call_retry_prompt(response)
        )
        retried = self._drop_unsupported_tool_calls(complete_once())
        if not _has_discarded_tool_calls(retried):
            return retried
        return ChatResponse(
            provider=retried.provider,
            model=retried.model,
            content=_discarded_tool_call_terminal_message(retried),
            finish_reason="error",
            usage=retried.usage,
    ...
    async def _retry_discarded_tool_calls_async(
        self,
        response: ChatResponse,
        complete_once,
    ) -> ChatResponse:
        ...
        retried = self._drop_unsupported_tool_calls(await complete_once())
        if not _has_discarded_tool_calls(retried):
            return retried
        return ChatResponse(
            provider=retried.provider,
    ...
    def _run_tool_loop_interactive(self, complete_once, *, initial_tool_choice="auto") -> AgentTurnResult:
        ...
        try:
            ...
            response = self._retry_discarded_tool_calls(response, complete_once)
            tool_rounds = 0
        ...
        response = self._append_final_response(response)
        self._compact_if_needed(trigger=ContextWindowTrigger.AUTO)
        return AgentTurnResult(status=AgentTurnStatus.COMPLETED, response=response)
    ...
    async def _run_tool_loop_interactive_async(self, complete_once, *, initial_tool_choice="auto") -> AgentTurnResult:
        ...
        try:
            ...
            response = await self._retry_discarded_tool_calls_async(response, complete_once)
            tool_rounds = 0
    ...
    def _continue_tool_loop_from_response(
        ...
        while response.tool_calls:
            ...
            if execution.stop_reason is not None:
                return self._limit_response(execution.stop_reason), None
    ...
    def _run_todo_self_check_if_needed(self, response: ChatResponse, complete_once) -> ChatResponse:
        if self._budget_finalization_requested:
            return response
    ...
    def _execute_tool_calls_interactive(self, tool_calls: list[ToolCall]) -> "_ToolExecutionState":
        ...
        while index < len(tool_calls):
            ...
            repeated = self._repeated_tool_failure_result(tool_call)
            if repeated is not None:
                self._emit_tool_event("finished", tool_call, result=repeated)
                self.session.append_tool_result(tool_call=tool_call, result=repeated)
                if repeated.data.get("terminal") is True:
                    self._append_skipped_tool_results(tool_calls[index + 1 :])
                    return _ToolExecutionState(
                        task_hash_changed=task_hash_changed,
                        stop_reason=AgentLoopStopReason.REPEATED_TOOL_FAILURE,
                    )
                index += 1
                continue
    ...
    def _parallel_readonly_batch_end(self, tool_calls: list[ToolCall], start: int) -> int:
        ...
        while end < len(tool_calls) and self._can_execute_in_parallel(tool_calls[end]):
            key = self._tool_call_key(tool_calls[end])
            if key in seen:
                break
            seen.add(key)
            end += 1
    ...
    def _parallel_tool_names_for_current_mode(self) -> frozenset[str]:
        if self.session.permission_manager is not None and self.session.permission_manager.mode == PermissionMode.FULL:
    ...
    def _execute_single_tool_call(self, tool_call: ToolCall) -> ToolResult:
        ...
        self._record_tool_result_for_repeat_guard(tool_call, result)
    ...
    def _repeated_tool_failure_result(self, tool_call: ToolCall) -> ToolResult | None:
        state = self._tool_failure_states.get(self._tool_call_key(tool_call))
        if state is None:
            return None
        state.attempts += 1
        terminal = state.attempts >= 3
        action = "本轮已停止。" if terminal else "请修改参数或更换工具策略。"
        return make_error_result(
            tool_call.name,
            (
                f"repeated_tool_failure\n相同工具调用已连续失败 {state.attempts} 次，"
                f"executor 未再次执行；{action}"
            ),
            error_code="repeated_tool_failure",
            original_error_code=state.error_code,
            failure_fingerprint=state.fingerprint,
            repeated_count=state.attempts,
            executor_skipped=True,
            terminal=terminal,
        )

    def _record_tool_result_for_repeat_guard(
        self,
        tool_call: ToolCall,
        result: ToolResult,
    ) -> None:
        ...
        if result.ok:
            if tool_call.name in _WORKSPACE_MUTATION_TOOL_NAMES:
                self._tool_failure_states.clear()
            else:
                self._tool_failure_states.pop(key, None)
            return
        if result.data.get("request_type") in {
            "permission_confirmation",
            "permission_denied",
        }:
            return
        if result.data.get("skipped_due_to_user_input"):
            return
        error_code = str(result.data.get("error_code") or "tool_error")
        result.data.setdefault("error_code", error_code)
        fingerprint = sha256(f"{key}\n{error_code}".encode("utf-8")).hexdigest()
        result.data.setdefault("failure_fingerprint", fingerprint)
        self._tool_failure_states[key] = _ToolFailureState(
    ...
    def _append_final_response(self, response: ChatResponse) -> ChatResponse:
        """在最终回答阶段拒绝未执行的 tool call，并保持历史可恢复。"""
        ...
        for tool_call in response.tool_calls:
            result = make_error_result(
                tool_call.name,
                "本轮已在安全停止边界结束，未执行模型额外返回的工具调用。",
                skipped_due_to_finalization=True,
            )
            self._emit_tool_event("skipped", tool_call, result=result)
            self.session.append_tool_result(tool_call=tool_call, result=result)
        final_response = ChatResponse(
            ...
            content="模型在最终回答阶段仍返回工具调用；Rook 已跳过这些调用并安全结束本轮。",
            tool_calls=[],
            finish_reason="stop",
        ...
        self.session.append_assistant_response(final_response)
        return final_response
    ...
    def _reserve_final_provider_call(self, tool_choice):
        """在达到硬上限前保留一次禁止工具的收尾请求。"""

        limit = self.limits.max_provider_calls
        if (
            not self.limits.reserve_final_provider_call
            or tool_choice == "none"
            or limit is None
            or limit < 2
            or self.provider_call_count != limit - 1
        ):
            return tool_choice
        self.session.append_runtime_control_message(
            "Provider 请求预算只剩最后一次。停止调用工具，基于现有工作区、验证结果和证据"
            "给出简洁最终回答；不得声称未验证的成功。"
        )
        self._budget_finalization_requested = True
        return "none"
    ...
    def _limit_response(self, reason: AgentLoopStopReason, *, raw: dict | None = None) -> ChatResponse:
        messages = {
            ...
            AgentLoopStopReason.REPEATED_TOOL_FAILURE: (
                "同一工具调用连续失败三次，Rook 已终止本轮，避免继续无效重试。"
            ),
        }
        return ChatResponse(
            provider=self.provider.name,

...
from rook_agent.agent.tool_flow import assistant_response_to_parts, tool_result_to_part
from rook_agent.context.identity import new_message_id
...
from rook_agent.context.system_prompt import PromptPrefixCache, SystemPromptBuilder
from rook_agent.context.task_boundary import observation_from_tool_result_data
from rook_agent.context.writer import SessionEventWriter
from rook_agent.permissions.grants import FilePermissionGrantStore, PermissionGrantStore
...
from rook_agent.permissions.types import PermissionDecisionKind, PermissionMode
from rook_agent.providers.types import ChatResponse, ProviderCapabilities, ToolCall
from rook_agent.permissions.types import PermissionDecision, PermissionRequest
from rook_agent.tools.permission_results import make_permission_denied_result
from rook_agent.tools.permission_registry import PermissionAwareToolRegistry
from rook_agent.tools.session_registry import ToolRegistryLike, create_session_tool_registry
...
class AgentSession:
    ...
    shell_guidance: str = WINDOWS_RESTRICTED_SHELL_GUIDANCE
    ...
    turn_counter: int = 0
    mode: str = PermissionMode.ASK.value
    ...
    def cancel_pending_permission_execution(self) -> bool:
        """结清被用户取消的权限请求，避免旧工具调用阻塞后续任务。"""
        ...
        self.append_tool_result(tool_call=pending.tool_call, result=result)
        for tool_call in pending.skipped_tool_calls:
            self.append_tool_result(
                tool_call=tool_call,
                result=make_error_result(
                    tool_call.name,
                    "当前任务已由用户取消，跳过同批次后续工具调用。",
                    skipped_due_to_user_cancellation=True,
                ),
            )
        self.pending_permission_execution = None
        return True
    ...
    def build_system_prefix(
        ...
        *,
        ...
        inputs = build_system_prompt_inputs(
            ...
            project_memory_context=self.project_memory_context,
            shell_guidance=self.shell_guidance,
    ...
    def _sync_sandbox_access_with_mode(self) -> None:
        if self.mode == PermissionMode.FULL.value:
...
def _task_boundary_required_stable_count(permission_manager: PermissionManager | None) -> int:
    if permission_manager is not None and permission_manager.mode == PermissionMode.FULL:
        return 1
    return 2

...
from rook_agent.session.models import SessionRecord
...
class SessionIndex:
    ...
    def update_event(self, event: SessionEvent) -> None:
        from rook_agent.session.catalog import _build_record_from_events
        ...
        if _is_user_visible(record):
            data["sessions"][event.session_id] = _record_to_dict(record)
        else:
            data["sessions"].pop(event.session_id, None)
    ...
    def rebuild(self) -> None:
        from rook_agent.session.catalog import _record_from_path
    ...
    def _reconcile_missing_files(self) -> None:
        ...
        changed = False
        for session_id, item in list(sessions.items()):
            if isinstance(item, dict) and not _is_user_visible(_record_from_dict(item)):
...
def _is_user_visible(record: SessionRecord) -> bool:
    return record.status == "corrupt" or record.message_count > 0

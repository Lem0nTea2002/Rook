...
catalog 从 `.rook/sessions/*.jsonl` 派生用户可见的 resume 列表。它不修复
...
from rook_agent.context.metadata import merge_metadata_patch
from rook_agent.session.errors import SessionInvalidIdError, SessionNotFoundError
...
class SessionCatalog:
    ...
    `root` 是 `.rook` 数据根目录，也就是 `JsonlSessionStore(root)` 使用的同一个
    ...
    def list_sessions(self) -> list[SessionRecord]:
        ...
        from rook_agent.session.index import SessionIndex

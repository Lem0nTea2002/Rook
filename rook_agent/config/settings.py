...
配置层是 Rook 的统一入口：CLI、provider factory、TUI 都应该通过这里拿到
...
class AppConfig:
    ...
    `env` 承载 ROOK_* 和厂商环境变量；`project_config` / `global_config`
    ...
    def get_provider_value(
        ...
        *,
        ...
        """按 Rook 配置优先级读取 provider 字段。

        优先级：环境变量 / `.env` > 项目 `rook.toml` > 全局配置 > 默认值。
        """
...
def load_config(
    ...
    *,
    ...
    selected_provider = (
        provider_name
        or env_snapshot.get("ROOK_PROVIDER")
...
def default_global_config_path() -> Path:
    ...
    if config_home:
        return Path(config_home) / "rook" / "config.toml"
    return Path.home() / ".config" / "rook" / "config.toml"

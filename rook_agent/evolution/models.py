...
class RecoveryTriggerKind(StrEnum):
...
class RecoveryOpportunityStatus(StrEnum):
    DETECTED = "detected"
    REVIEWED = "reviewed"
    SAVED = "saved"
    DISMISSED = "dismissed"
...
class LearningDestination(StrEnum):
...
def load_evolution_config(config: AppConfig) -> EvolutionConfig:
    ...
    return EvolutionConfig(
        enabled=_bool_value_from_raw(
            config.get_section_value("evolution", "enabled", default=True)
        ),
        scope=scope,

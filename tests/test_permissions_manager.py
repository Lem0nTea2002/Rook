...
def test_manager_falls_back_to_mode_aware_policy(tmp_path) -> None:
    ...
    decision = manager.preflight(
        PermissionRequest(id="req_1", action=PermissionAction.WRITE_PATH, target="rook_agent/new.py")
    )
...
def test_manager_deny_grant_still_overrides_aggressive_policy(tmp_path) -> None:
        ...
        grants=PermissionGrantStore(
            [
                PermissionGrant(
                    ...
                    scope_value=str(tmp_path / "rook_agent"),
                    created_at="2026-06-04T00:00:00+08:00",
                )
            ]
        ),
    ...
    decision = manager.preflight(
        PermissionRequest(
            id="req_1",
            action=PermissionAction.WRITE_PATH,
            target=str(tmp_path / "rook_agent" / "new.py"),
        )
    )
...
def test_manager_full_mode_ignores_legacy_deny_grant(tmp_path) -> None:
    manager = PermissionManager(
        policy=DefaultPermissionPolicy(tmp_path),
        grants=PermissionGrantStore(
            [
...
def test_manager_builds_permission_confirmation_request(tmp_path) -> None:
    ...
    assert [option.id for option in confirmation.options] == [
        ...
        PermissionConfirmationChoice.ALLOW_SESSION_SAME_SCOPE.value,
...
def test_manager_resolves_session_grant_without_persisting_it(tmp_path) -> None:
    persistent = PermissionGrantStore()
    manager = PermissionManager(
        policy=DefaultPermissionPolicy(tmp_path),
        grants=persistent,
    ...
    decision = manager.resolve_confirmation(request, "allow_session_same_scope")
    later = manager.preflight(
        PermissionRequest(
            id="perm_shell_later",
            action=PermissionAction.EXECUTE_SHELL,
    ...
    assert decision.kind == PermissionDecisionKind.ALLOW
    assert decision.persistence == PermissionPersistence.SESSION
    assert later.kind == PermissionDecisionKind.ALLOW
    assert later.grant == decision.grant
    assert persistent.list() == []


def test_full_mode_ignores_persistent_denies(tmp_path) -> None:
    manager = PermissionManager(
        policy=DefaultPermissionPolicy(tmp_path),
        grants=PermissionGrantStore(
            [
...
def test_full_mode_does_not_call_permission_policy(tmp_path) -> None:
    class ExplodingPolicy(DefaultPermissionPolicy):
        def decide(self, request, *, mode):
            raise AssertionError("FULL 不应调用权限策略")

    manager = PermissionManager(
        policy=ExplodingPolicy(tmp_path),
        mode=PermissionMode.FULL,
    )
...
def test_manager_can_create_explicit_grant_for_sensitive_env_after_confirmation(tmp_path) -> None:
    manager = PermissionManager(policy=DefaultPermissionPolicy(tmp_path))

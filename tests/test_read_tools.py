...
def test_grep_finds_matching_lines(tmp_path):
    source = tmp_path / "rook_agent.py"
    source.write_text("alpha\nRook agent\nbeta\n", encoding="utf-8")
    registry = create_builtin_registry(tmp_path)

    result = registry.execute("grep", {"pattern": "Rook", "include": "*.py"})

    assert result.ok is True
    assert result.data["results"][0]["path"] == "rook_agent.py"
    assert result.data["results"][0]["line"] == 2


def test_grep_with_rg_filters_sensitive_environment(tmp_path, monkeypatch):
    source = tmp_path / "rook_agent.py"
    source.write_text("Rook agent\n", encoding="utf-8")
    captured = {}
    ...
    monkeypatch.setenv("ROOK_VISIBLE_TEST_FLAG", "visible")
    registry = create_builtin_registry(tmp_path)
    ...
    assert captured["env"]["ROOK_VISIBLE_TEST_FLAG"] == "visible"


def test_grep_falls_back_to_python_when_rg_is_missing(tmp_path, monkeypatch):
    (tmp_path / "src").mkdir()
    source = tmp_path / "src" / "rook_agent.py"
    source.write_text("alpha\nRook agent\nbeta\n", encoding="utf-8")
    ...
    assert result.data["results"][0]["path"] == "src/rook_agent.py"
    assert result.data["results"][0]["line"] == 2
...
def test_grep_parses_rg_output_for_windows_paths(tmp_path):
    source = tmp_path / "rook_agent.py"
    source.write_text("Rook agent", encoding="utf-8")
    sandbox = PathSandbox(tmp_path)
    output = f"{source}:1:Rook agent\n"

    results = grep_module._parse_rg_output(sandbox, output, max_results=10)

    assert results == [{"path": "rook_agent.py", "line": 1, "text": "Rook agent"}]


def test_glob_finds_matching_paths(tmp_path):
    (tmp_path / "rook_agent").mkdir()
    (tmp_path / "rook_agent" / "app.py").write_text("print('hi')", encoding="utf-8")
    (tmp_path / "README.md").write_text("# readme", encoding="utf-8")
    registry = create_builtin_registry(tmp_path)
    ...
    assert result.ok is True
    assert result.data["matches"] == ["rook_agent/app.py"]

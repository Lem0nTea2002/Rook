...
from rook_agent.eval.adapter import RetryableBenchmarkProvider
...
from rook_agent.providers.types import ChatRequest, ToolCall
...
def test_adapter_builds_benchmark_prompt(tmp_path: Path):
    ...
    assert result.model_name_or_path == "rook-test"
...
def test_default_loop_factory_keeps_session_outside_repo(tmp_path: Path):
    ...
    outside.write_text("must stay isolated", encoding="utf-8")
    outside_result = loop.session.tool_registry.execute(
        "view",
        {"path": str(outside)},
    )
    assert outside_result.ok is False
    assert "must stay isolated" not in outside_result.content
...
def test_default_loop_factory_can_omit_todo_for_micro_benchmark(tmp_path: Path):
    repo = tmp_path / "repo"
    repo.mkdir()
    init_repo(repo)
    adapter = RookCodingAgentAdapter(
    ...
    assert "todo" not in loop.session.tool_registry.names()
...
def test_relative_session_root_is_resolved_outside_repo(tmp_path: Path, monkeypatch):
    ...
    adapter = RookCodingAgentAdapter(loop_factory=make_loop)
    task = CodingTask(
        instance_id="sympy__sympy-20590",
        repo_path=repo,
        problem_statement="Fix the issue.",
    ...
    assert seen_session_roots == [tmp_path / ".rook-eval" / "sympy__sympy-20590"]
...
def test_session_root_inside_repo_is_rejected(tmp_path: Path):
    ...
    repo.mkdir()
    init_repo(repo)
    adapter = RookCodingAgentAdapter(session_root="repo/.rook-eval")
    task = CodingTask(
        instance_id="sympy__sympy-20590",
        repo_path=repo,
        problem_statement="Fix the issue.",

...
def test_deepseek_preset_declares_thinking_tool_compatibility() -> None:
    capabilities = PROVIDER_PRESETS["deepseek"].capabilities

    assert capabilities.supports_tool_choice_parameter is False
...
def test_deepseek_replays_reasoning_content_for_tool_calls() -> None:
    ...
    provider.complete(
        ChatRequest(
            ...
            tools=[ToolDefinition(name="read_file", description="读取文件", parameters={})],
        )
    )

    assistant = client.completions.last_params["messages"][0]
    assert assistant["reasoning_content"] == "先读取文件。"
    assert "tool_choice" not in client.completions.last_params
...
def test_openai_compatible_provider_can_ignore_system_proxy(monkeypatch) -> None:
    ...
    monkeypatch.setattr("openai.DefaultHttpxClient", create_http_client)
    monkeypatch.setattr("openai.OpenAI", create_openai_client)

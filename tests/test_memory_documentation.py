...
def test_readmes_publish_memory_benchmark_boundary_and_commands() -> None:
    ...
    assert "## Project-memory benchmark" in english
    assert "## 项目记忆评测" in chinese
    for text in (english, chinese):
        ...
        assert "0%" in text
    assert "the 0% / 0% Pilot is not a resume metric" in english
    assert "0% / 0% Pilot 不能作为简历指标" in chinese

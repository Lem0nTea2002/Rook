...
def test_run_chain_commits_model_and_keeps_test_patch_out_of_agent_workspace(tmp_path: Path):
    ...
    assert not list(tmp_path.glob("rook-chainswe-verifier-*"))
...
def test_rook_chain_agent_uses_project_scoped_auto_no_network_tools_and_swe_limits(tmp_path: Path):
    repo = tmp_path / "repo"
    _init_repo(repo)
    ...
    assert agent.app.current_session.session.mode == "auto"
    assert agent.app.current_session.session.sandbox_access.unrestricted is False

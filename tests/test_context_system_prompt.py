from rook_agent.context.system_prompt import (
    PromptPrefixCache,
    SystemPromptInputs,
    SystemPromptBuilder,
)
...
def _inputs(**overrides: object) -> SystemPromptInputs:
    values = {
        "base_rules": "你是 Rook。",
        "agents_md": "项目规则：上下文放在 rook_agent/context。",
...
def test_system_prompt_cache_reuses_prefix_when_fingerprint_matches() -> None:
    ...
    assert "你是 Rook。" in first.messages[0].content
...
def test_system_prompt_contains_english_agent_behavior_rules() -> None:
    ...
    assert "Keep the required mutation separate from auxiliary verification" in content
    ...
    assert "ROOK_POST_WRITE_VERIFICATION_INCONCLUSIVE" in content

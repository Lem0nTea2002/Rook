...
def test_session_index_registers_session_only_after_first_message(tmp_path: Path) -> None:
    store = JsonlSessionStore(tmp_path)
    store.append_event(
        SessionEvent(
            id="evt_created",
    ...
    assert SessionIndex(tmp_path).list_records() == []
    ...
    store.append_event(
        SessionEvent(
            ...
            payload={
                "message_id": "msg_user",
                "parts": [{"id": "part_user", "kind": "text", "content": "开始任务"}],
            },
            created_at="2026-06-01T00:00:01Z",
        )
    )

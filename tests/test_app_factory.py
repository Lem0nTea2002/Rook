...
def test_create_rook_app_wires_session_commands_context_and_chat(tmp_path: Path) -> None:
    ...
    assert isinstance(app.chat_runner.candidate_coordinator, LearningCoordinator)
    ...
    assert app.command_handler.handle("/sessions").output == "No sessions."
...
def test_headless_runtime_can_forbid_full_access_mode(tmp_path: Path) -> None:
    runtime = create_rook_runtime(
        project_root=tmp_path,
        data_root=tmp_path / ".rook",
        provider=FakeProvider([]),
    ...
    result = runtime.command_handler.handle("/mode full --confirm")

    assert result.handled is True
    assert "仅限本地 TUI/CLI" in result.output
    assert runtime.current_session.mode == "ask"
...
def test_new_session_does_not_inherit_full_access(tmp_path: Path) -> None:
    app = create_rook_app(
        project_root=tmp_path,
        data_root=tmp_path / ".rook",
        provider=FakeProvider([]),
    ...
    app.current_session.session.set_permission_mode(PermissionMode.FULL)

    result = app.command_handler.handle("/new safe")

    assert result.output.startswith("New session: sess_")
    assert app.current_session.session.mode == PermissionMode.ASK.value
    assert app.current_session.session.sandbox_access.unrestricted is False

...
from rook_agent.tools.permission_registry import PermissionAwareToolRegistry, permission_request_for_tool
from rook_agent.tools.registry import ToolRegistry
from rook_agent.tools.types import Tool, ToolPermissionSpec, make_text_result
...
def test_permission_aware_registry_asks_before_sensitive_env_read(tmp_path) -> None:
    calls: list[str] = []
    registry = PermissionAwareToolRegistry(
        ToolRegistry(
            [
...
def test_invalid_network_tool_arguments_fail_before_permission_preflight(tmp_path) -> None:
    calls: list[dict[str, object]] = []
    ...
    result = registry.execute("web_search", {"query": "Rook", "max_chars": 6000})

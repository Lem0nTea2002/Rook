...
from rook_agent.app.runtime import AgentChatRunner, CurrentSessionState, _display_lines_from_messages
...
from rook_agent.providers.types import ChatRequest, ChatResponse, ChatStreamEvent, ProviderDiagnostics, ToolCall
...
from rook_agent.tools.types import make_text_result, Tool
...
def test_agent_chat_runner_cancel_clears_pending_input(tmp_path) -> None:
    store = JsonlSessionStore(tmp_path)
    session = AgentSession.create(store=store, session_id="sess_test", agents_md="")
...
def test_agent_chat_runner_exposes_pending_user_input(tmp_path) -> None:
    ...
    assert coordinator.after_sessions == []
...
def test_cancelled_permission_does_not_block_retry_after_switching_to_full(tmp_path) -> None:
    store = JsonlSessionStore(tmp_path / ".rook")
    session = AgentSession.from_project(
        store=store,
        session_id="sess_cancel_permission_then_full",
        project_root=tmp_path,
        tools=[create_write_tool(tmp_path)],
    )
    provider = FakeProvider(
        [
            ChatResponse(
                ...
                tool_calls=[
                    ToolCall(
                        id="call_write_cancelled",
                        name="write",
                        arguments={"path": "README.md", "content": "old"},
                    )
                ],
                finish_reason="tool_calls",
            ),
    ...
    runner = AgentChatRunner(
        current_session=CurrentSessionState(session),
        provider=provider,
        task_boundary_decider=lambda _: "same",
    )

    waiting = runner.run_user_turn("第一次写入")
    assert waiting.finish_reason == AgentTurnStatus.WAITING_FOR_USER_INPUT.value
    assert session.pending_permission_execution is not None

    runner.cancel_current_turn()
    assert session.pending_permission_execution is None
    assert session.restore_pending_permission_execution() is None
    assert not (tmp_path / "README.md").exists()

    session.set_permission_mode(PermissionMode.FULL)
    response = runner.run_user_turn("重新写入")

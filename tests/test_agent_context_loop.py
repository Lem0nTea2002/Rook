...
from rook_agent.agent import loop as agent_loop
...
from rook_agent.context.manager import ContextCompactResult, ContextWindowTrigger
...
from rook_agent.tools.types import Tool, ToolResult
...
def test_agent_loop_persists_provider_diagnostics_metadata(tmp_path) -> None:
    ...
    assert response.content == "已使用合法参数继续。"
...
def test_agent_loop_skips_tool_calls_returned_during_final_only_completion(
    tmp_path,
) -> None:
    store = JsonlSessionStore(tmp_path)
    session = AgentSession.create(
        store=store,
        session_id="sess_verify_final_tool_call",
        agents_md="",
    )
    provider = FakeProvider(
        [
            ChatResponse(
                ...
                content="",
                tool_calls=[
                    ToolCall(id="call_test", name="shell", arguments={})
                ],
                finish_reason="tool_calls",
            ),
            ChatResponse(
                ...
                content="",
                tool_calls=[
                    ToolCall(id="call_extra", name="shell", arguments={})
                ],
                finish_reason="tool_calls",
            ),
        ]
    ...
    view = store.rebuild_session_view("sess_verify_final_tool_call")
    validate_tool_call_sequence(view.messages)
    assert response.tool_calls == []
    assert "已跳过" in response.content
    skipped = [
        part
        for message in view.messages
        for part in message.parts
        if part.metadata.get("tool_call_id") == "call_extra"
        and part.kind == "tool_result"
    ]
    assert skipped[0].metadata["data"]["skipped_due_to_finalization"] is True
...
def test_agent_loop_reserves_last_provider_call_for_final_answer(tmp_path) -> None:
    store = JsonlSessionStore(tmp_path)
    session = AgentSession.create(
        store=store,
        session_id="sess_provider_final",
        agents_md="",
        tools=[create_todo_tool()],
    )
    provider = FakeProvider(
        [
            ChatResponse(
                ...
                tool_calls=[
                    ...
                    ToolCall(id="call_echo", name="echo", arguments={"text": "work"}),
                ],
                finish_reason="tool_calls",
            ),
            ChatResponse(provider="fake", model="fake-model", content="已安全收尾。", finish_reason="stop"),
        ]
    )
    ...
    ).run_user_turn("调用工具后结束")

    assert response.content == "已安全收尾。"
    assert response.finish_reason == "stop"
    assert len(provider.requests) == 2
    assert provider.requests[0].tool_choice == "auto"
    assert provider.requests[1].tool_choice == "none"
    assert "停止调用工具" in provider.requests[1].messages[-1].content


def test_streaming_agent_loop_reserves_last_provider_call_for_final_answer(tmp_path) -> None:
    store = JsonlSessionStore(tmp_path)
    session = AgentSession.create(
        store=store,
        session_id="sess_stream_provider_final",
        agents_md="",
        tools=[create_todo_tool()],
    )
    provider = StreamingProvider(
    ...
    ).run_user_turn_streaming_sync("调用工具后结束")

...
from rook_agent.context.content.build import BuildOutputRouteCompressor
from rook_agent.context.content.code import SourceCodeRouteCompressor
from rook_agent.context.content.compressors import PlainTextRouteCompressor
from rook_agent.context.content.diff import GitDiffRouteCompressor
from rook_agent.context.content.html import HtmlRouteCompressor
from rook_agent.context.content.json import JsonRouteCompressor
from rook_agent.context.content.search import SearchResultsRouteCompressor
...
def test_route_compact_dispatches_by_content_type() -> None:
    ...
    part = _part("rook_agent/app.py:10:def run():\nrook_agent/app.py:20:def stop():")
...
def test_route_compact_rejects_output_that_is_not_smaller() -> None:
    original = "rook_agent/app.py:10:def run():"
...
def test_route_compact_adds_metadata() -> None:
    ...
    result = router.compact_part(_part("rook_agent/app.py:10:def run():\n" * 20))
...
def test_search_results_compressor_groups_files_and_records_omissions() -> None:
    content = "\n".join(
        [
            "rook_agent/app.py:1: def old_1(): pass",
            *[f"rook_agent/app.py:{line}: def old_{line}(): pass with repeated context" for line in range(2, 40)],
            "rook_agent/app.py:3: TODO important path",
            "rook_agent/tools.py:10: normal match",
            *[f"rook_agent/tools.py:{line}: normal match with repeated context" for line in range(12, 50)],
            "rook_agent/tools.py:11: ERROR must keep this",
        ]
    )
    router = RouteCompactRouter(
    ...
    assert "rook_agent/app.py:1:" in result.content
    assert "rook_agent/app.py:3: TODO important path" in result.content
    assert "rook_agent/tools.py:11: ERROR must keep this" in result.content
    assert "omitted" in result.content


def test_search_results_compressor_parses_windows_paths() -> None:
    content = "\n".join(
        rf"C:\repo\rook_agent\app.py:{line}: def run_{line}(): pass with repeated context"
    ...
    assert r"C:\repo\rook_agent\app.py:1:" in result.content


def test_search_results_compressor_keeps_sentinel_like_matches() -> None:
    content = "\n".join(
        [
            *[f"rook_agent/context/manager.py:{line}: normal match {line}" for line in range(1, 180)],
            "rook_agent/context/manager.py:180: SEARCH_SENTINEL_9 important routing evidence",
            *[f"rook_agent/context/manager.py:{line}: trailing match {line}" for line in range(181, 360)],
        ]
    )
    router = RouteCompactRouter(
...
def test_git_diff_compressor_keeps_headers_changes_and_limited_context() -> None:
    content = "\n".join(
        [
            "diff --git a/rook_agent/app.py b/rook_agent/app.py",
            "--- a/rook_agent/app.py",
            "+++ b/rook_agent/app.py",
            "@@ -1,25 +1,25 @@",
    ...
    assert "diff --git a/rook_agent/app.py b/rook_agent/app.py" in result.content
...
def test_git_diff_compressor_counts_unified_diff_without_git_header() -> None:
    ...
    assert "--- a/rook_agent/app.py" in result.content
    assert "+++ b/rook_agent/app.py" in result.content
...
def test_html_compressor_extracts_visible_content_and_links() -> None:
    ...
    assert result.metadata["html_title"] == "Rook Notes"

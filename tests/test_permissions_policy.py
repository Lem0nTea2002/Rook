...
def test_legacy_standard_maps_to_ask_and_requires_project_root_read_confirmation(tmp_path) -> None:
...
def test_aggressive_allows_plain_project_write_but_not_sensitive_path(tmp_path) -> None:
    ...
    plain = policy.decide(
        _request(PermissionAction.WRITE_PATH, "rook_agent/new_file.py"),
...
def test_aggressive_does_not_auto_allow_delete(tmp_path) -> None:
    ...
    inside = policy.decide(
        _request(PermissionAction.DELETE_PATH, "rook_agent/old.py"),
        mode=PermissionMode.AGGRESSIVE,
    )
    ...
    assert outside.kind == PermissionDecisionKind.ASK


def test_sensitive_env_requires_confirmation_outside_full_mode(tmp_path) -> None:
    policy = DefaultPermissionPolicy(tmp_path)
...
def test_bypass_mode_allows_permission_requests_without_prompting(tmp_path) -> None:
    ...
    requests = (
        ...
        _request(PermissionAction.WRITE_PATH, ".env"),
        _request(PermissionAction.DELETE_PATH, str(tmp_path.parent / "outside.py")),
        _request(PermissionAction.EXECUTE_SHELL, "rm -rf rook_agent", cwd=tmp_path),
...
def test_non_sensitive_env_requires_confirmation(tmp_path) -> None:
    ...
    decision = policy.decide(
        _request(PermissionAction.READ_ENV, "ROOK_MODE"),
        mode=PermissionMode.AGGRESSIVE,
    )
...
def test_readonly_git_inside_project_is_allowed_in_auto_mode(tmp_path) -> None:
    policy = DefaultPermissionPolicy(tmp_path)
...
def test_auto_does_not_treat_arbitrary_interpreters_or_git_apply_as_safe(tmp_path) -> None:
    policy = DefaultPermissionPolicy(tmp_path)

    for command in (
        "python scripts/export.py",
        "python3 scripts/inspect_sqlite.py",
        "sqlite3 shop.db DELETE FROM users",
        "git apply p1.patch",
    ):
        decision = policy.decide(
            _request(PermissionAction.EXECUTE_SHELL, command, cwd=tmp_path),
...
def test_ask_mode_requires_confirmation_for_every_permission_action(tmp_path) -> None:
    policy = DefaultPermissionPolicy(tmp_path)
    requests = (
        _request(PermissionAction.READ_PATH, "README.md"),
        _request(PermissionAction.WRITE_PATH, "README.md"),
        _request(PermissionAction.DELETE_PATH, "README.md"),
        _request(PermissionAction.EXECUTE_SHELL, "pytest -q", cwd=tmp_path),
        _request(PermissionAction.NETWORK_REQUEST, "https://example.com"),
        _request(PermissionAction.READ_ENV, "ROOK_MODE"),
        _request(PermissionAction.GIT_OPERATION, "status", cwd=tmp_path),
    )

    for request in requests:
        decision = policy.decide(request, mode=PermissionMode.ASK)
        assert decision.kind == PermissionDecisionKind.ASK, request


def test_auto_mode_allows_only_project_local_safe_actions(tmp_path) -> None:
    policy = DefaultPermissionPolicy(tmp_path)

    allowed = (
        _request(PermissionAction.READ_PATH, "README.md"),
        _request(PermissionAction.WRITE_PATH, "rook_agent/new.py"),
        _request(PermissionAction.EXECUTE_SHELL, "pytest -q", cwd=tmp_path),
        _request(PermissionAction.GIT_OPERATION, "status", cwd=tmp_path),
    )
    asked = (
        _request(PermissionAction.READ_PATH, str(tmp_path.parent / "outside.txt")),
        _request(PermissionAction.WRITE_PATH, str(tmp_path.parent / "outside.txt")),
        _request(PermissionAction.DELETE_PATH, "README.md"),
        _request(PermissionAction.EXECUTE_SHELL, "rm README.md", cwd=tmp_path),
        _request(PermissionAction.NETWORK_REQUEST, "https://example.com"),
        _request(PermissionAction.READ_ENV, "OPENAI_API_KEY"),
        _request(PermissionAction.GIT_OPERATION, "reset --hard", cwd=tmp_path),
    )

    for request in allowed:
        assert policy.decide(request, mode=PermissionMode.AUTO).kind == PermissionDecisionKind.ALLOW
    for request in asked:
        assert policy.decide(request, mode=PermissionMode.AUTO).kind == PermissionDecisionKind.ASK


def test_legacy_permission_mode_names_resolve_to_public_modes() -> None:
    assert PermissionMode("conservative") is PermissionMode.ASK
    assert PermissionMode("standard") is PermissionMode.ASK
    assert PermissionMode("aggressive") is PermissionMode.AUTO
    assert PermissionMode("bypass") is PermissionMode.FULL

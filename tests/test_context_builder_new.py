...
from rook_agent.context.system_prompt import SystemPromptBuilder, SystemPromptInputs
...
def test_context_builder_projects_internal_messages_to_provider_messages() -> None:
    ...
    messages = ContextBuilder().build_provider_messages(
        view,
        system_prefix=[ChatMessage(role="system", content="你是 Rook。")],
    )
...
def test_context_builder_preserves_assistant_reasoning_for_provider_replay() -> None:
    view = SessionView(
        session_id="sess_test",
        messages=[
            AgentMessage(
                ...
                metadata={"diagnostics": {"reasoning": "先读取文件。"}},
            ),
            AgentMessage(
                id="msg_tool",
                session_id="sess_test",
    ...
    assert messages[0].reasoning_content == "先读取文件。"
...
def test_context_builder_accepts_stable_system_prefix_from_builder() -> None:
    prefix = SystemPromptBuilder().build(
        SystemPromptInputs(
            base_rules="你是 Rook。",
            agents_md="上下文管理放在 rook_agent/context。",
    ...
    assert "你是 Rook。" in messages[0].content
    assert "basis_message_id=msg_user" in messages[1].content

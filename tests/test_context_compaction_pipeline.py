...
from rook_agent.context.archive import ToolResultArchive
from rook_agent.context.checkpoint import Checkpoint
from rook_agent.context.compaction import CompactionPipeline, CompactionRequest
from rook_agent.context.models import AgentMessage, MessagePart, SessionView
from rook_agent.context.tool_sequence import validate_tool_call_sequence
...
def test_l2_routes_derived_search_and_stores_raw_backing(tmp_path: Path) -> None:
    raw_content = "\n".join(
        f"rook_agent/app.py:{line}: def function_{line}(): pass" for line in range(1, 160)
    )
    view = SessionView(
        session_id="sess_test",
        messages=[
...
def test_l2_never_routes_fresh_source_and_does_not_create_backing(tmp_path: Path) -> None:
    ...
    view = SessionView(
        session_id="sess_test",
        messages=[
            _tool_call("view_l2_fresh", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 500}),
            _tool_result(
                "view_l2_fresh",
                "view",
                content=raw_content,
                data={
                    "path": "rook_agent/context.py",
                    "start_line": 1,
                    "end_line": 500,
                    "total_lines": 2_000,
                    "truncated": True,
...
def test_l2_routes_build_and_diff_derived_results_with_raw_backing(tmp_path: Path) -> None:
    ...
    view = SessionView(
        ...
        messages=[
            ...
            _tool_call("diff_l2", "git_diff", {"path": "rook_agent/app.py"}),
...
def test_per_result_pressure_does_not_bypass_fresh_source_noop(tmp_path: Path) -> None:
    ...
    view = SessionView(
        session_id="sess_test",
        messages=[
            _tool_call("pressure_fresh", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 500}),
            _tool_result(
                "pressure_fresh",
                "view",
                content=raw_content,
...
def test_l3_keeps_fresh_large_view_and_structured_tool_call_byte_identical(tmp_path: Path) -> None:
    call = _tool_call("view_fresh", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 500})
...
def test_l3_archives_stale_view_and_keeps_raw_backing(tmp_path: Path) -> None:
    ...
    view = SessionView(
        session_id="sess_test",
        messages=[
            _tool_call("view_stale", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 500}),
            _tool_result(
                "view_stale",
                "view",
                content=raw_content,
            ...
            _tool_call("edit_stale", "edit", {"path": "rook_agent/context.py"}),
            _tool_result("edit_stale", "edit", content="updated", data={"path": "rook_agent/context.py"}),
        ],
    )
...
def test_l3_archives_superseded_view_after_later_covering_view(tmp_path: Path) -> None:
    ...
    view = SessionView(
        session_id="sess_test",
        messages=[
            _tool_call("view_first", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 100}),
            _tool_result(
                "view_first",
                "view",
                content=first_content,
                data={"path": "rook_agent/context.py", "start_line": 1, "end_line": 100, "total_lines": 500, "truncated": True},
            ),
            _tool_call("view_second", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 500}),
            _tool_result(
                "view_second",
                "view",
                content=second_content,
                data={"path": "rook_agent/context.py", "start_line": 1, "end_line": 500, "total_lines": 500, "truncated": False},
            ),
        ],
    )
...
def test_required_l3_runs_below_budget_and_repeat_is_idempotent(tmp_path: Path) -> None:
    ...
    source_view = SessionView(
        session_id="sess_test",
        messages=[
            _tool_call("view_required", "view", {"path": "rook_agent/context.py", "offset": 0, "limit": 20}),
            _tool_result(
                "view_required",
                "view",
                content=raw_content,
                data={"path": "rook_agent/context.py", "start_line": 1, "end_line": 20, "total_lines": 200, "truncated": True},
            ),
            _tool_call("write_required", "write", {"path": "rook_agent/context.py"}),
            _tool_result("write_required", "write", content="updated", data={"path": "rook_agent/context.py"}),
        ],
    )
    pipeline = CompactionPipeline(root=tmp_path)
    request = CompactionRequest(
...
def test_task_switch_compaction_saves_context_without_breaking_tool_transaction(tmp_path: Path) -> None:
    ...
    old_search_output = "\n".join(
        f"rook_agent/old_module.py:{line}: obsolete_symbol_{line}" for line in range(1, 500)
    )

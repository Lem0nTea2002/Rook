...
def test_create_provider_from_config_can_disable_sdk_retries_for_benchmark() -> None:
    config = AppConfig(
        provider_name="deepseek",
        env={"DEEPSEEK_API_KEY": "test-key"},
        global_config={"model": "deepseek-v4-flash"},
    )

    provider = create_provider_from_config(config, transport_max_retries=0)

    assert isinstance(provider, OpenAICompatibleProvider)
    assert provider.transport_max_retries == 0
    assert provider.base_url == "https://api.deepseek.com"
    assert provider.capabilities.supports_tools is True


def test_create_provider_from_config_can_disable_system_proxy_trust() -> None:
    config = AppConfig(
        provider_name="deepseek",
        env={"DEEPSEEK_API_KEY": "test-key"},
        global_config={
            "model": "deepseek/deepseek-v4-flash",
            "provider": {"type": "deepseek", "trust_env": False},
        },
    )
...
def test_create_provider_rejects_literal_secret_in_api_key_env() -> None:
    literal_secret = "sk-live-secret-value"
    config = AppConfig(
        provider_name="deepseek",
        env={"DEEPSEEK_API_KEY": "stored-secret"},
        global_config={
            "provider": {
                "type": "deepseek",
    ...
    assert "rook config setup --force" in message

...
def test_default_plan_selects_default_chain_and_requires_only_key_name(tmp_path: Path):
    ...
    plan = make_run_plan(
        chains_path=dataset,
        source_root=source,
        environ={"ROOK_GPT56_API_KEY": "super-secret"},
    )
...
def test_docker_command_uses_official_image_and_never_exposes_hidden_chain_data(tmp_path: Path):
    ...
    assert f"src={source},dst=/opt/rook-src,readonly" in rendered
    ...
    assert "ROOK_PROVIDER=openai-compatible" in rendered
    assert "ROOK_MODEL=gpt-5.6-terra" in rendered
...
def test_bootstrap_uses_target_python_for_agent_venv_and_keeps_verifier_path_clean():
    ...
    assert 'tar -C /opt/rook-src' in bootstrap
    ...
    assert "printenv \"$ROOK_CHAIN_API_KEY_ENV\"" in bootstrap
    assert "ROOK_GPT56_API_KEY" not in bootstrap
...
def test_selected_chain_is_serialized_only_for_stdin(tmp_path: Path):
    ...
    plan = make_run_plan(
        chains_path=dataset,
        source_root=source,
        environ={"ROOK_GPT56_API_KEY": "secret"},
    )

    ...
    result = registry.execute(
        "python_exec",
        {
            "code": (
                ...
                "print(os.environ.get('ROOK_VISIBLE_TEST_FLAG', '<missing>'))"
            ),
        },
    )
...
def test_diagnostics_failure_exposes_bounded_output_to_agent(monkeypatch, tmp_path):
    def fake_run(command, **kwargs):
        return diagnostics_module.subprocess.CompletedProcess(
            command,
            1,
    ...
    assert result.ok is False
    assert "诊断命令退出码为 1" in result.content
    assert "FAILED tests/test_example.py::test_case" in result.content
    assert "assert 1 == 2" in result.content

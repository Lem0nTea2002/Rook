...
| `PermissionMode` | public `ask`, `auto`, or `full` operating mode |
...
- `ask`: asks for every permission-bearing action;
- `auto`: allows ordinary in-project file access, read-only Git, and an explicit
  safe command list;
- `full`: skips permission confirmation after an explicit local session warning
  while retaining redacted audit events.

Legacy `conservative`, `standard`, `aggressive`, and `bypass` values migrate to
`ask`, `ask`, `auto`, and `full`. Full access is available only through the
local `/permissions` flow and cannot be inherited by mobile channels, EvalOps,
or Candidate sandboxes.

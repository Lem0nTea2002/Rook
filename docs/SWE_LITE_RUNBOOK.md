...
prepared instance repositories + issue JSONL
  -> Rook prediction generation
  -> predictions JSONL (patches)
  -> official SWE-bench Docker harness
  -> resolved/not-resolved report
```

The Rook command generates patches. The official harness evaluates those
...
- Use an instances JSONL containing `instance_id`, `repo`, `base_commit`, and
  `problem_statement`.
- Configure the Rook provider and put generated sessions/predictions in a
...
```sh
.venv/bin/python -m rook_agent.eval.swebench \
  --instances data/swebench_lite_one.jsonl \
  --repos-root /tmp/rook-swe-lite/repos \
  --out runs/rook_swe_lite_predictions.jsonl \
  --provider openai \
  --model-name rook \
  --max-instances 1 \
...
repository, runs `RookCodingAgentAdapter`, and writes one official-format
row for each result:

```json
{"instance_id":"...","model_name_or_path":"rook","model_patch":"diff --git ..."}
```
...
python -m swebench.harness.run_evaluation \
  ...
  --predictions_path runs/rook_swe_lite_predictions.jsonl \
  --max_workers 1 \
  --run_id rook-swe-lite
```
...
These validate Rook's generation contract, not a live Docker score.

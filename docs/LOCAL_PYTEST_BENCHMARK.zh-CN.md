...
这是 Rook 的快速本地 coding-agent 探针。JSONL 的每一行会生成一个干净的小 Git 仓库；agent 接题、修改仓库，runner 执行该题声明的 pytest 命令。它测的是“读代码、改代码、跑测试、收尾”的真实闭环，不是只测模型能不能说出一段像样的话。

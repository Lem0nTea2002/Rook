...
Rook 的命令只生成 patch，官方 harness 才负责判 patch。生成了 JSONL 不等于评测完成；agent 自己跑的测试也不等于官方分数。
...
- 配好 Rook provider，session/prediction 放可丢弃的 `runs/`。
...
该命令会为每个选中 instance 创建 `CodingTask`、要求仓库干净、运行 `RookCodingAgentAdapter`、并写出每项一行的官方格式：
...
它验证 Rook 的生成契约，不代表真实 Docker 分数。

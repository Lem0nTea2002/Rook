...
护栏给这笔事务设上限，避免模型绕圈、provider 太慢、工具过多时无限续杯。它们是 `rook_agent/agent/loop.py` 里的代码检查，不是 system prompt 里喊一句“请不要循环”就能实现的。
...
`view`、`grep`、`git_diff` 等只读调用在响应允许时可以并发；FULL mode 中还有一份明确的更宽并发名单。写入顺序不会被随手并行。
...
```sh
rg -n "TOOL_ROUND_LIMIT|max_provider_calls|prompt too long|PendingPermission" tests rook
```
...
**“FULL 把 wrapper 删了。”** 没有。它跳过权限确认；session registry、事件记录、结构化结果、loop limit 都还在。

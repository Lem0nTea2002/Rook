...
| 3 | `~/.agents/skills`、`~/.codex/skills`、`~/.rook/skills` | 全局 agent/markdown skill |
| 4 | `ROOK_SKILL_ROOTS` 的逗号分隔目录 | 额外全局根 |

`<project>/skills/INDEX.md` 是目录说明，不是可执行 skill。设 `ROOK_DISABLE_GLOBAL_SKILLS=1` 可仅发现项目 skill。frontmatter 可提供 `name`、`description`、逗号分隔 `triggers`；结果会排序去重，避免同一根重复出现导致目录不稳定。

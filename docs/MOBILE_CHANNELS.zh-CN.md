...
Rook v0.4.2 可以在电脑上运行一个本地常驻 Gateway，让已配对的手机飞书或
...
```powershell
rook channel project add rook --path "D:\WorkAndStudy\FindJob\Rook"
```
...
普通文本会作为 Coding Task。手机入口不提供 `!shell`、bypass、永久授权、项目
配置或白名单修改能力。

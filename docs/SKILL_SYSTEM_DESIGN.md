...
| 3 | `~/.agents/skills`, `~/.codex/skills`, `~/.rook/skills` | global agent/markdown skill |
| 4 | `ROOK_SKILL_ROOTS` comma-separated roots | additional global roots |

`<project>/skills/INDEX.md` is catalog text, not a runnable skill. Set
`ROOK_DISABLE_GLOBAL_SKILLS=1` to keep discovery project-only. Frontmatter

...
rook_agent/cli.py
  ...
  -> append facts to .rook session JSONL
...
1. **Name the behavior.** Search the visible command, tool name, or error using
   `rg -n "term" rook tests`. Avoid guessing from filenames.
...
Open `rook_agent/agent/loop_limits.py`, change nothing, and run the relevant

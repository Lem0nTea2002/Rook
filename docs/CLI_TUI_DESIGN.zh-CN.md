...
这篇解释 Rook 怎样启动、怎样组装出可用 agent session、以及运行时事件怎样变成终端上的内容。它不定义工具安全和厂商协议；那两部分请看文末链接。
...
rook [flags]
  ...
  -> 事实持久化留在 <project>/.rook
```

例如 `rook --message "explain loop limits"` 走同步单轮路径；Textual 界面则通过 `AgentChatRunner` 走异步流式路径。两者共用 session、tool registry、provider type 和 agent loop，但展示、打断和确认交互不同。

## 启动与依赖装配

主组合根是 `rook_agent/app/factory.py:create_rook_app`，按顺序创建：

1. 默认位于 `<project>/.rook` 的 `JsonlSessionStore`（可由 `data_root` 覆盖）。
...
8. `AgentChatRunner`、`RuntimeModelSwitcher`，最后才是 `RookApp`。
...
`rook_agent/cli.py` 当前会路由这些模式：

| 调用方式 | 适用场景 | 关键限制 |
| --- | --- | --- |
| `rook` 或 `--tui` | 完整 Textual 交互 | 需要交互终端 |
...
`app/tui.py` 的 `RookApp` 渲染 `app/tui_state.py` 的 transcript 型状态：对话条目、工具活动、todo、provider/session 状态和 pending input。它会先缓冲 token，再批量刷新，避免一个 token 刷一次 widget。
...
```sh
.venv/bin/python -m rook_agent --help

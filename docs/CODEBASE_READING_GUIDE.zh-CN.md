...
```text
rook_agent/cli.py
  -> rook_agent/app/factory.py:create_rook_app
  -> rook_agent/app/runtime.py:AgentChatRunner
  -> rook_agent/agent/loop.py:AgentLoop
  -> ContextBuilder 组装 ChatRequest(messages, tools)
  -> provider.complete/astream
  -> session.tool_registry.execute
  -> 事实追加写入 .rook 的 session JSONL
  -> runtime 发送界面更新
```
...
1. **先命名行为。** 用 `rg -n "关键词" rook tests` 搜可见命令、工具名或错误，不要先猜文件。
...
```sh
rg -n "class AgentLoop|def create_rook_app" rook tests
rg -n "ChatRequest\(|tool_registry\.execute|preflight\(" rook tests
...
**“FULL 就是没有审计代码。”** 不是。它跳过权限确认；工具仍经 registry，并返回结构化结果。

**“压缩把历史删了。”** 不是。JSONL 事实还在；checkpoint 改的是本次 provider 看见的视图。

## 一个上手练习

打开 `rook_agent/agent/loop_limits.py`，先不改任何代码，用 `rg -n "max_tool_rounds|TOOL_ROUND_LIMIT" tests` 找到测试并运行，再沿 stop reason 回读 `loop.py`。这就是本项目推荐的工作方式：先说清一个小不变量、用测试确认、再只改拥有该职责的最小层。

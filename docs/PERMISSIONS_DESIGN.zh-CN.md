...
| `PermissionMode` | 对外公开的 `ask`、`auto`、`full` |
...
- `ask`：所有受权限系统管理的动作都询问；
- `auto`：只自动允许项目内普通文件访问、只读 Git 和明确安全命令；
- `full`：本地会话显式确认后跳过权限确认，但继续保留脱敏审计。

旧配置 `conservative`、`standard`、`aggressive`、`bypass` 会分别迁移为
`ask`、`ask`、`auto`、`full`。`full` 只能从本地 `/permissions` 进入，
不能被飞书、微信、EvalOps 或 Candidate 沙箱继承。

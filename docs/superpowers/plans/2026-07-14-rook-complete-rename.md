...
**Files:**
- Rename: current repository directory to `D:\WorkAndStudy\FindJob\New-Harness-Agent\Rook`
...
```powershell
& 'D:\WorkAndStudy\FindJob\New-Harness-Agent\Rook\.venv\Scripts\python.exe' -m pytest 'D:\WorkAndStudy\FindJob\New-Harness-Agent\Rook\tests\test_brand_contract.py' -q
```

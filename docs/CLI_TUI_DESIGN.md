...
This document explains how Rook starts, assembles a usable agent session,
...
rook [flags]
  ...
  -> durable events remain in <project>/.rook
```

For example, `rook --message "explain loop limits"` uses a synchronous
...
`rook_agent/app/factory.py:create_rook_app` is the main composition root.
In order, it creates:

1. `JsonlSessionStore` under `<project>/.rook` unless `data_root` is set.
...
8. `AgentChatRunner`, `RuntimeModelSwitcher`, and finally `RookApp`.
...
`rook_agent/cli.py` routes these modes:

| Invocation | Intended use | Important limitation |
| --- | --- | --- |
| `rook` or `--tui` | full interactive Textual UI | requires an interactive terminal |
...
`RookApp` in `app/tui.py` renders a transcript-oriented state model from

...
shape to Rook's in-process benchmark adapter.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Rook on the current harness-bench workspace.")
    ...
    parser.add_argument("--model-name", default="rook-harness-bench-fast")
    parser.add_argument("--session-root", default=".rook-harness")
    return parser
...
def _ensure_git_repo(workspace: Path) -> None:
    ...
    (workspace / ".gitignore").write_text(".rook-harness/\n__pycache__/\n*.py[cod]\n.pytest_cache/\n", encoding="utf-8")

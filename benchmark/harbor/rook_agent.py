"""Harbor installed-agent adapter for Rook.

The adapter stages only the local Rook package into each task container,
...
_AGENT_ROOT: Final = "/opt/rook-agent"
_REMOTE_SOURCE_DIR: Final = "/installed-agent/rook-src"
_SESSION_ROOT: Final = "/tmp/rook-harbor-sessions"
_DEFAULT_PACKAGE: Final = (
    "https://github.com/KomorGiaoGiao/Rook/archive/refs/heads/main.zip"
)


class RookHarborAgent(BaseInstalledAgent):
    """Run the current Rook checkout as a Harbor installed agent.
    ...
    def get_version_command(self) -> str | None:
        return (
            ...
            "\"from importlib.metadata import version; print(version('rook-agent'))\""
        )

    @override
    async def install(self, environment: BaseEnvironment) -> None:
        """Install Rook into an isolated venv inside the task container."""
    ...
    async def run(
        ...
        """Run one Rook benchmark turn in Harbor's configured workdir."""

        del context  # Rook persists its own local benchmark transcript.
        ...
        await self.exec_as_agent(
            ...
            env={"ROOK_DISABLE_GLOBAL_SKILLS": "1"},
        )
    ...
    def _stage_local_source(self) -> Path:
        ...
        if source is None:
            raise ValueError(
                "No local Rook source directory is available. Pass "
                "source_dir=... or package=... to RookHarborAgent."
            )
        package_dir = source / "rook_agent"
        required = (source / "pyproject.toml", source / "README.md", package_dir)
        missing = [str(path) for path in required if not path.exists()]
        if missing:
            raise ValueError(
                "Rook source directory is incomplete; missing " + ", ".join(missing)
            )

        staged = self.logs_dir / "rook-source"
        if staged.exists():
            shutil.rmtree(staged)
        staged.mkdir(parents=True)
        shutil.copy2(source / "pyproject.toml", staged / "pyproject.toml")
        shutil.copy2(source / "README.md", staged / "README.md")
        shutil.copytree(package_dir, staged / "rook_agent", ignore=_ignore_source_artifacts)
        return staged
    ...
    def _run_command(self, instruction: str, *, session_id: str) -> str:
        ...
        return (
            f"{shlex.quote(_venv_python())} -m rook_agent "
            "--benchmark --project . "

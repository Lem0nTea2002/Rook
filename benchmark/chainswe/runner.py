...
class RookChainAgent:
    """Production ChainSWE agent with one app and one persistent session.

    ``create_rook_app`` is called once per chain.  Every ``run_task``
    therefore appends to the same Rook session, allowing the benchmark to
    ...
    def __init__(
        ...
        *,
        ...
        access = SandboxAccess(mode=SandboxAccessMode.PROJECT)
        ...
        permission_manager = self.app.current_session.session.permission_manager
        if permission_manager is None:
            raise RuntimeError("ChainSWE 运行器缺少权限管理器。")
        permission_manager.policy = ChainSWEBenchmarkPermissionPolicy(self.workspace)
        self.app.current_session.set_permission_mode(PermissionMode.AUTO)
    ...
    def _rebuild_chain_session_tool_registry(self) -> None:
        ...
        App construction injects session-scoped tools before benchmark mode is
        ...
        only that session registry after selecting AUTO with the benchmark-specific
...
class ChainSWEBenchmarkPermissionPolicy(DefaultPermissionPolicy):
    """仅允许 ChainSWE 在当前仓库内写入和执行本地命令。"""
...
class _TemporaryVerifierWorktree:
    ...
    def __enter__(self) -> Path:
        ...
        path = Path(tempfile.mkdtemp(prefix="rook-chainswe-verifier-", dir=self.repo.parent))
        path.rmdir()
        self.path = path
...
def _commit_all(repo: Path, message: str) -> str:
    ...
    _run_checked(
        [
            "git",
            "-c",
            "user.name=Rook ChainSWE",
            "-c",
            "user.email=chainswe@rook.local",
            "commit",
            "--allow-empty",
            "-m",
            message,
...
def _create_provider(
    *,
    ...
    if model:
        config = replace(config, env={**config.env, "ROOK_MODEL": model})
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one persistent Rook session over a ChainSWE chain.")
    ...
    parser.add_argument(
        "--data-root",
        default=".rook-chainswe-data",
        help="Session data directory. Relative paths resolve beside the workspace, never inside it.",
    )
    parser.add_argument("--provider", default=None, help="Rook provider override.")
    parser.add_argument("--model", default=None, help="Rook model override.")

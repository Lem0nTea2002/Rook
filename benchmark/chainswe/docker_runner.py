...
This is deliberately a host-side launcher, not part of Rook's agent
...
container bootstrap maps that value to ``ROOK_API_KEY`` at runtime.
"""
...
DEFAULT_API_KEY_ENV = "ROOK_GPT56_API_KEY"
CONTAINER_SOURCE_ROOT = "/opt/rook-src"
CONTAINER_RUN_ROOT = "/runs"
CONTAINER_PYTHON = "/opt/conda/envs/swebench_matterhorn/bin/python"
CONTAINER_AGENT_VENV = "/opt/rook-agent/.venv"
CONTAINER_AGENT_SOURCE = "/opt/rook-agent/src"
...
if [ -z \"${{ROOK_API_KEY:-}}\" ]; then
  ROOK_API_KEY=\"$(printenv \"$ROOK_CHAIN_API_KEY_ENV\" || true)\"
  export ROOK_API_KEY
fi
if [ -z \"${{ROOK_API_KEY:-}}\" ]; then
...
def build_docker_command(plan: DockerRunPlan) -> list[str]:
    ...
    return [
        ...
        "--env",
        f"ROOK_PROVIDER={plan.provider}",
        "--env",
        f"ROOK_PROVIDER_NAME={plan.provider_name}",
        "--env",
        f"ROOK_MODEL={plan.model}",
        "--env",
        f"ROOK_BASE_URL={plan.base_url}",
        "--env",
        f"ROOK_CHAIN_API_KEY_ENV={plan.api_key_env}",
        "--env",
        plan.api_key_env,
...
def make_run_plan(
    *,
    ...
    if not source.is_dir() or not (source / "pyproject.toml").is_file():
        raise ValueError(f"Rook source root is not a project directory: {source}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--provider", default=DEFAULT_PROVIDER, help="Rook provider override.")
    parser.add_argument("--provider-name", default=DEFAULT_PROVIDER_NAME, help="OpenAI-compatible provider display name.")
    parser.add_argument("--model", default=DEFAULT_MODEL, help="Rook model override.")
    ...
    parser.add_argument("--max-tool-rounds", type=int, default=None, help="Optional Rook turn limit override.")
    return parser

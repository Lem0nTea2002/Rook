...
`runner.py` owns the persistent Rook session, model commits, and isolated
...
your Rook GPT-5.6 configuration, then launch it:

```sh
export ROOK_GPT56_API_KEY="..."
...
- `session-data/`: the persistent Rook session data for the chain.
...
record once through Docker standard input before Rook starts. This keeps
...
key variable:  ROOK_GPT56_API_KEY
```
...
and the bootstrap maps it to Rook's standard runtime variable inside the
...
`/opt/rook-agent/.venv`, copies the read-only mounted Rook project
